<script lang="ts">
	// from https://svelte.dev/repl/c7094fb1004b440482d2a88f4d1d7ef5?version=4.2.0
	...
	let newOptions: { value: string; name: string | null }[] = [];
	...
	onMount(() => {
		slot.querySelectorAll('option').forEach((o) => {
			o.selected && !value.includes(o.value) && (value = [...value]);
	...
	function handleKeyup(e) {
		if (e.keyCode === 13) {
			if (!activeOption) {
				...
				const newOption = { value: inputValue, name: inputValue };
			...
			const allOptions = filtered.concat(newOptions);
			const calcIndex = allOptions.indexOf(activeOption) + increment;
			// activeOption =
			// 	calcIndex < 0
			...
			switch (true) {
				case calcIndex < -1:
					if (newOptions.length) {
						activeOption = newOptions.at(-1);
					} else {
						activeOption = filtered.at(-1);
					}
					return;
				case calcIndex === -1:
					input.focus();
					activeOption = null;
					return;
				case calcIndex === allOptions.length:
					input.focus();
					activeOption = null;
					return;
				default:
					if (calcIndex < filtered.length) {
						activeOption = filtered[calcIndex];
					} else {
						activeOption = newOptions[calcIndex - filtered.length];
					}
			}
		} else {
			activeOption = undefined;
...
<div class="multiselect" class:readonly>
	...
	<select bind:this={slot} multiple class="hidden">
		<slot />
		{#each newOptions as newOption}
			<option value={newOption.value}>{newOption.name}</option>
		{/each}
	</select>
	...
	{#if showOptions}
		...
		>
			...
			{#each newOptions as newOption}
				<li
					class:selected={selected[newOption.value]}
					class:active={activeOption?.value === newOption.value}
