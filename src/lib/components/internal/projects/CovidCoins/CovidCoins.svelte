<script lang="ts">
	import { onMount } from 'svelte';
	import { csv } from 'd3';

	import Chart from './Chart/Chart.svelte';
	import type { HandleChangeEvent, Outcome, RiskItem } from './constants';
	import { dataFolder } from './constants';
	import { processAgeItem, processMortalityItem } from './logic';

	let data: RiskItem[] | null = null;

	let outcome: Outcome = 'mortality';
	let by: 'mortalityAge' | 'disabilityInfectionCount' = 'mortalityAge';
	let input: number = 40;

	let baselineMortality: RiskItem[];
	let mortalityByAge: RiskItem[];
	let mortalityByAgeCovid: RiskItem[];

	let baselineDisability: RiskItem[];
	let disabilityByReinfectionCovid: RiskItem[];

	onMount(async () => {
		baselineMortality = await csv(`${dataFolder}/micromortsBaseline.csv`, processMortalityItem);
		mortalityByAge = await csv(`${dataFolder}/mortalityByAge.csv`, processAgeItem);
		mortalityByAgeCovid = await csv(`${dataFolder}/mortalityByAgeCovid.csv`, processAgeItem);

		baselineDisability = await csv(`${dataFolder}/disabilityBaseline.csv`, processMortalityItem);
		disabilityByReinfectionCovid = await csv(
			`${dataFolder}/disabilityByReinfectionCovid.csv`,
			processMortalityItem,
		);

		data = [...baselineMortality, mortalityByAge[input], mortalityByAgeCovid[input]];
	});

	const handleOutcomeChange = (event: HandleChangeEvent<HTMLSelectElement>) => {
		const target = event.target as HTMLSelectElement;
		outcome = target.value as Outcome;
		if (outcome === 'mortality') {
			by = 'mortalityAge';
			input = 40;
			data = [...baselineMortality, mortalityByAge[input], mortalityByAgeCovid[input]];
		} else {
			by = 'disabilityInfectionCount';
			input = 0;

			data = [...baselineDisability, disabilityByReinfectionCovid[input]];
		}
	};

	const handleAgeInputChange = (event: HandleChangeEvent<HTMLInputElement>) => {
		const target = event.target as HTMLInputElement;
		const value = Number(target.value);
		const outOfRange = value < 20 || value > 100;
		if (isNaN(value) || outOfRange) {
			return;
		}

		input = value;
		data = [...baselineMortality, mortalityByAge[input], mortalityByAgeCovid[input]];
	};

	const handleReinfectionDisabilityChange = (event: HandleChangeEvent<HTMLInputElement>) => {
		const target = event.target as HTMLInputElement;
		const value = Number(target.value);
		const outOfRange = value < 0 || value > 2;
		if (isNaN(value) || outOfRange) {
			return;
		}

		input = value;
		data = [...baselineDisability, disabilityByReinfectionCovid[input]];
	};
</script>

<div class="inputs-container">
	<h3>Chance of</h3>
	<select on:input={handleOutcomeChange} value={outcome}>
		<option value="mortality">Death</option>
		<option value="disability">Injury/Long Covid</option>
	</select>
	<h3>by</h3>
	<select bind:value={by}>
		{#if outcome === 'mortality'}
			<option value="mortalityAge">Age</option>
		{:else}
			<option value="disabilityInfectionCount">Infection Count</option>
		{/if}
	</select>
	{#if by === 'mortalityAge'}
		<h3>if you are</h3>
	{:else if by === 'disabilityInfectionCount'}
		<h3>if you've been infected</h3>
	{/if}
	{#if by === 'mortalityAge'}
		<input on:input={handleAgeInputChange} value={input} type="number" min="20" max="100" />
	{:else if by === 'disabilityInfectionCount'}
		<input
			on:input={handleReinfectionDisabilityChange}
			value={input}
			type="number"
			min="0"
			max="2"
		/>
	{/if}
	{#if by === 'disabilityInfectionCount'}
		<h3>time{input === 1 ? '' : 's'} before</h3>
	{/if}
</div>

<div class="chart-container">
	{#if data}
		<Chart {data} {outcome} />
	{:else}
		<h1>LOADING</h1>
	{/if}
</div>

<style>
	.inputs-container {
		display: flex;
		flex-wrap: wrap;
		row-gap: var(--spacing-8);
		column-gap: var(--spacing-16);
		margin-bottom: var(--spacing-16);
	}

	.chart-container {
		margin-bottom: var(--spacing-24);
	}
</style>
